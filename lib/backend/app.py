...
OVERLAP = 10
DEBUG_MODE = True
PROFILE_BUCKET = "palm-models"  # Replace this with your Supabase Storage bucket name
...
# === RoboFlow helper ===
def call_roboflow(image_bytes, model_url, api_key):
...
def detect_hand():
    ...
    img_bytes = file.read()
    result = call_roboflow(img_bytes, HAND_MODEL_URL, HAND_API_KEY)
...
def detect_sift_features(image_np):
    ...
    sift = cv2.SIFT_create(nfeatures=2000, contrastThreshold=0.01, edgeThreshold=5, sigma=1.2)
...
def segment_lines():
    try:
        ...
        result = call_roboflow(img_bytes.read(), LINE_MODEL_URL, LINE_API_KEY)
